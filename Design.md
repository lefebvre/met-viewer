...
**Compression caveat — szip/AEC is not supported.** hdf5 is built without its `szip` feature and eccodes without `aec`, so neither szip-compressed NetCDF-4/HDF5 nor AEC-packed GRIB2 can be decoded; such a file surfaces as a decode error rather than bad data. Both features exist only to pull in `libaec`, which is fetched from `gitlab.dkrz.de` — a host that rate-limits archive downloads hard enough (HTTP 429, reproduced off-CI and unaffected by newer vcpkg ports, which only rename the project path) to fail the build outright. In practice the formats this app targets use deflate: ERA5 and NOAA NetCDF-4 do, and GRIB2 AEC packing was already unsupported before this. Re-enabling is a one-line revert of the `hdf5` entry in `vcpkg.json` if the host recovers or a user needs it.
...
- **The suite runs on Windows as well as Linux.** The tempting assumption is that the logic is platform-identical and one run suffices; it is not. `long` is 32-bit on MSVC and 64-bit on Linux, which is the whole reason the ARL reader carries int64 record offsets and the GRIB reader a `seek64` helper — a Linux-only suite cannot regress-test either, and a wrapped offset shows up as silently misread data rather than a crash. Running there needs the vcpkg `bin` directory on `PATH` and `QT_PLUGIN_PATH` pointed at the Qt plugins (the triplet is dynamic, and `met_app_tests` starts a QApplication), both set before ctest because `gtest_discover_tests(PRE_TEST)` enumerates by running the executables. The two `Warp.*PerformanceTripwire` tests are excluded there: wall-clock budgets are a useful regression signal on the consistent Linux runner and pure flakiness on a shared Windows one.
- **Per-run artifacts.** Windows CI stages `cmake --install` into a directory and uploads it (14-day retention), which is self-contained because the install rules run windeployqt and drop PROJ's data at `share/proj`, where `locateBundledProjData()` looks. Linux deliberately does not do the same: its install tree is *not* self-contained — bundling Qt there is what linuxdeploy does when it builds the AppImage on a tag — so an equivalent upload would produce something that only runs on a machine that already has the right Qt.
- Every commit and every CI run: `ctest --preset asan` (unit + golden-image + warp-benchmark tests). CI builds `dist-linux` with `-DMET_WERROR=ON` and then re-runs the suite under address/undefined sanitizers via the `asan-dist` preset — the warning set is only a gate if something fails on it. Two things that are easy to get wrong here: the sanitizer job needs `libasan`/`libubsan` installed (GCC ships only the compiler half), and the sanitizer preset must inherit `dist-linux` rather than use the plain `asan` one, or it picks up the default triplet and builds a second complete dependency set.
- **Host triplet is pinned to the target triplet** on the dist-linux preset. Because `x64-linux-dynamic` differs from vcpkg's default host triplet `x64-linux`, vcpkg classified the build as a cross-build and produced a *second* dependency set for host tools: 42 extra packages, including an entire second Qt built with the port's default features (icu, sql-psql, testlib, dbus, xcb-sm) rather than the trimmed set we ask for. Nothing from that set ships, it roughly doubled CI time, and it eventually broke the build outright — its `xcb-sm` default requires libSM, which the CI container has no reason to install. Setting `VCPKG_HOST_TRIPLET` to the target removes the whole duplicate set (79 planned packages → 48) and with it the failure.
- **CI dependency caching.** vcpkg binaries and downloaded distfiles are cached separately, via the `files` binary-cache provider plus `actions/cache`. The previous `x-gha` provider was removed from vcpkg and — importantly — its removal is *not* an error: vcpkg prints a warning and proceeds with no cache, so every run silently rebuilt all 80 packages from source and re-downloaded every tarball, which eventually earned an HTTP 429 from one dependency's upstream host. Two design notes follow from that: restore and save are separate steps with `if: always()`, so a run that dies partway still leaves its work for the next one (the combined action only saves on success); and `vcpkg/downloads` is cached on its own looser key, so a binary-cache miss does not re-ask upstream for every distfile.
