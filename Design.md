...
- **M7 — Animation & polish** *(risk #3)* — DONE: FieldCache (LRU, user-set byte budget, decode-ahead prefetch of upcoming timesteps) + in-app playback (TimeController play/pause + fps QTimer stepping the time axis, cache-backed so frames are instant) + QSettings persistence (window state, colormap/basemap/opacity/overlays, cache size, fps) + Preferences dialog. **Deferred:** the GL LUT fast path (`QOpenGLWidget` + R32F/LUT shader) — the CPU warp (~38 ms/1080p, from M3) plus the field cache already give smooth playback on typical grids, so the GL path is a future optimization, not needed for the milestone. MP4/GIF export also deferred (the `--grab` frame capture exists for stills).
