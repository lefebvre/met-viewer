...
- **M7 — Animation & polish** *(risk #3)*: FieldCache (user-set size) + decode-ahead, GL LUT fast path, ≥10 fps on hourly ERA5, session/settings persistence + preferences dialog. **Animation = in-app playback** (TimeController steps the loaded time axis and re-renders live layers — fully interactive, pan/zoom/probe while playing); a frame-capture **export to MP4/GIF** (pipe QImages to ffmpeg) is an optional M7 stretch item, not the mechanism. *Done: 48-frame animation plays smoothly while panning.*
